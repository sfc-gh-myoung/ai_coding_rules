**Description:** Core Streamlit patterns for setup, navigation, state management, and deployment modes in Snowflake
...
- **Type:** Agent Requested
- **Scope:** Streamlit core application architecture, multipage navigation, deployment modes, configuration management
...
- **Output Format:** Streamlit app with proper navigation, consistent theming via config.toml, secure configuration, responsive layout using native components
- **Validation Steps:** Test navigation flow, verify theme loaded from config.toml, validate responsive design, check secrets loading, test multipage state persistence
...
<section_metadata>
  <section_id>deployment</section_id>
