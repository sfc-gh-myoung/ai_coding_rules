**Description:** Security patterns, input validation, and secrets management for Streamlit applications
...
- **Type:** Agent Requested
- **Scope:** Streamlit security, input validation, secrets management, authentication
...
- **Output Format:** Secure Streamlit app with validated inputs, secrets management, error handling that doesn't expose sensitive information
- **Validation Steps:** Test with invalid inputs (should reject), verify secrets not exposed in UI/logs, test file upload limits, check error messages don't expose credentials
...
```python
# ✓ Validated numeric input with bounds
age = st.number_input(
    "Age", 
    min_value=0, 
    max_value=120, 
