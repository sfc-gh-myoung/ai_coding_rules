**Description:** Git workflow best practices for GitHub and GitLab covering branching strategies, merge workflows, protected branches, and git state validation.
**AppliesTo:** `.git/**/*`, `**/.gitignore`, `**/.gitattributes`
**AutoAttach:** false
...
- **Inputs/Prereqs:** Git repository initialized; remote configured (GitHub or GitLab); understanding of Conventional Commits; access to Pre-Task-Completion Validation Gate requirements
- **Allowed Tools:** Git commands (`git branch`, `git checkout`, `git commit`, `git push`, `git status`); GitHub CLI (`gh`); GitLab CLI (`glab`); validation commands
- **Forbidden Tools:** Direct commits to protected branches without PR/MR; force push to `main`/`master` without override; git commands that bypass validation
- **Required Steps:**
...
- **Output Format:** Clean git history with semantic commits; properly named branches; complete PR/MR descriptions
- **Validation Steps:** Git state validation passes; branch name follows convention; CHANGELOG.md updated; all status checks pass; code review approved
...
- **Contributing Workflow**: `805-project-contributing-rules.md`
