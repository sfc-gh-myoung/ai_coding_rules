...
- **feat(rules)!:** **Breaking:** tighten rule body schema to v3.4. `## Scope` MUST contain bolded inline labels `**What This Rule Covers:**` and `**When to Load This Rule:**`; `## References` MUST contain H3 subsections `### Dependencies` and `### External Documentation` (use `_None._` when empty). The transient `### Related Rules` subsection experiment introduced and reverted mid-session is now retired; 35 rules that carried a `### Related`, `### Related Rules`, or `### Related Examples` heading pre-v3.4 had those sections removed (token bloat with minimal value). Backfilled 74 rule files via `.workbench/scripts/rule_schema_backfill.py`; every touched file bumps `RuleVersion` PATCH and stamps `LastUpdated: 2026-07-04`.
- **feat(validate):** `ai-rules validate rules/` now enforces the v3.4 body schema via a new `_validate_body_sections` check: HIGH-severity errors fire on missing Scope inline labels or missing References subheadings, using `_normalize_section_name` for parenthetical-tolerant matching. Extends the existing `RULES_INDEX.md` skip list to also skip `RULES_INDEX_COMPACT.md` and its template.
- **feat(schema):** `schemas/rule-schema.yml` bumped to `version: "3.4"`; `structure.required_sections[References].content_validation.required_subsections` unchanged at `["Dependencies", "External Documentation"]`; description updated.
- **docs(rules/002):** `002-rule-governance.md` §Required Sections rewritten for v3.4 with an explicit enforcement note; SchemaVersion bumped v3.3 → v3.4.
- **docs(rules/002a):** update `002a-rule-creation.md` for v3.4 schema — every `v3.3` reference bumped to `v3.4`, SchemaVersion frontmatter v3.3 → v3.4, RuleVersion PATCH v3.3.2 → v3.3.3.
- **docs(rules/002b):** update `002b-rule-update.md` for v3.4 schema and add a PATCH-bump note covering schema-conformance body-section backfills; SchemaVersion v3.3 → v3.4, RuleVersion v1.2.3 → v1.2.4.
- **docs(new):** bump `ai-rules new rule` scaffold generator docstrings to advertise v3.4 template compliance (template body already had the required v3.4 sections and labels).
