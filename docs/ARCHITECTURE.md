...
5. **Agent-Agnostic Deployment.** A single `--dest` flag deploys to any project structure.
...
│   (validation)   (validate,         (split or      (Claude,       │
│                   index, deploy)     unified)       Cursor, …)    │
│                                                                   │
│   skills/  ─────────────────────────────────────► (agent          │
...
├── 002-rule-governance.md (+002a-002m companions)
...
├── Resolve target paths (unified vs split)
├── Render AGENTS.md template (with rules/skills path substitution)
├── Copy rules/ → DEST/rules/  (excluded skills filtered)
├── Copy skills/ → DEST/skills/ (optional)
...
For deployment commands and split-mode configuration, see [README.md → Quick Start](../README.md#quick-start).
