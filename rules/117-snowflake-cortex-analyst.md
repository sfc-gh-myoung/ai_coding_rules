...
**SchemaVersion:** v3.0
**Keywords:** semantic views, natural language queries, agent tool configuration, semantic view design, Generator workflow, error troubleshooting, business metrics, analyst not working, fix analyst, debug analyst, semantic view validation, text to SQL
**TokenBudget:** ~4650
**ContextTier:** Medium
**Depends:** rules/100-snowflake-core.md, rules/106-snowflake-semantic-views-core.md, rules/105-snowflake-cost-governance.md, rules/111-snowflake-observability-core.md
...
Semantic Views modeling; Cortex Analyst configuration and usage; agent tool integration; integration with AISQL/Agents/Search; governance
...
<mandatory>
Semantic Views, Cortex Analyst, AISQL for enrichment, Cortex Search for retrieval, Cortex Agents
</mandatory>
...
<output_format>
Semantic view definitions, agent tool configurations, usage patterns
</output_format>

<validation>
Views compile; definitions align to business glossary; access rules enforced; analyst prompts produce consistent answers; tool descriptions are clear
</validation>
...
- [ ] Semantic Views define explicit measures/dimensions and grain
      Verify: `SHOW SEMANTIC VIEWS;` then check DDL for measures/dimensions with clear grain
- [ ] No `SELECT *`; comments present for non-obvious fields
      Verify: Review semantic view DDL - search for "SELECT *" and check COMMENT clauses
- [ ] NO flagging/threshold logic in semantic views (belongs in agent instructions)
      Verify: Search semantic view DDL for CASE WHEN statements - should be none
- [ ] Masking and row access policies applied where needed
      Verify: `SHOW MASKING POLICIES;` and `SHOW ROW ACCESS POLICIES;` - check coverage
- [ ] Naming conventions and fully qualified names used
      Verify: Check semantic view uses DATABASE.SCHEMA.TABLE format throughout
- [ ] Tool descriptions include clear when-to-use guidance
      Verify: Review agent config tool descriptions - should specify analytical domain
- [ ] Tool use cases are distinct (no overlaps with other tools)
      Verify: Compare tool descriptions across all analyst tools in agent config
- [ ] Prompt templates documented for Cortex Analyst
      Verify: Check agent planning instructions include query patterns and examples
- [ ] Component testing completed for analyst tools
      Verify: Test semantic view with TABLE(SEMANTIC_VIEW()) before agent integration
...
- **Semantic Views**: `rules/106-snowflake-semantic-views-core.md` - Comprehensive semantic view DDL syntax, development best practices (Section 8), Generator workflow, testing patterns
...
- **Cortex Search**: `rules/116-snowflake-cortex-search.md` - Search tool configuration for hybrid agents
...
```sql
-- Verify Generator is enabled (requires appropriate Snowflake edition and region)
SHOW PARAMETERS LIKE '%SEMANTIC_VIEW_GENERATOR%' IN ACCOUNT;

-- Check account capabilities
SELECT 
    SYSTEM$GET_ACCOUNT_CAPABILITIES() AS capabilities;

-- Expected: JSON response should include semantic view generation capabilities
-- If Generator not available, proceed with manual CREATE SEMANTIC VIEW DDL
...
-- Example: Identify columns for FACTS vs DIMENSIONS
SELECT 
    COLUMN_NAME,
    DATA_TYPE,
    IS_NULLABLE,
...
-- Required grants:
-- - CREATE VIEW on schema (for regular views)
-- - CREATE SEMANTIC VIEW privilege (for native semantic views)
-- - SELECT on source tables
-- - USAGE on database and schema
-- - USAGE on CORTEX functions (for Cortex Analyst integration)
...
**Verify Semantic View Structure Quality:**
```sql
-- After creating semantic view, validate quality

-- Check semantic view exists
SHOW SEMANTIC VIEWS IN SCHEMA {DATABASE}.{SCHEMA};

-- Validate semantic view components
...
SHOW SEMANTIC FACTS IN SEMANTIC VIEW {DATABASE}.{SCHEMA}.{VIEW_NAME};
...
-- Expected: Query executes successfully and returns meaningful results
```
...
**RECOMMENDED:**
**Follow iterative development workflow from `106-snowflake-semantic-views-core.md` Section 8:**

1. **Start with Generator** (if available): Use Semantic View Generator to create baseline DDL
2. **Validate structure**: Review generated FACTS vs DIMENSIONS classification
3. **Add business context**: Include WITH SYNONYMS and COMMENT clauses
4. **Test with Cortex Analyst**: Validate natural language query accuracy
5. **Optimize performance**: Ensure base tables have appropriate clustering/partitioning

**Cross-Reference:**
- **Generator workflow**: See `106-snowflake-semantic-views-core.md` Section 8.1
- **Iterative development**: See `106-snowflake-semantic-views-core.md` Section 8.2
- **Testing patterns**: See `106-snowflake-semantic-views-core.md` Section 8.3

```sql
-- Example: explicit columns and comments; avoid SELECT *
-- For native semantic views, use CREATE SEMANTIC VIEW (preferred)
CREATE OR REPLACE SEMANTIC VIEW ANALYTICS.SEMANTIC.customer_orders
  TABLES (
    orders AS ANALYTICS.CORE.ORDERS
      PRIMARY KEY (order_id)
      WITH SYNONYMS ('orders', 'sales', 'transactions')
  ...
  DIMENSIONS (
    orders.order_id AS order_id
      COMMENT = 'Surrogate key of order',
    orders.customer_id AS customer_id
      WITH SYNONYMS ('customer', 'buyer'),
    orders.order_date AS order_date
      WITH SYNONYMS ('date', 'transaction date', 'purchase date')
  ...
  COMMENT = 'Customer order semantic view for Cortex Analyst natural language queries';

-- For legacy views (not native semantic views):
CREATE OR REPLACE VIEW ANALYTICS.SEMANTIC.customer_orders_legacy
...
- **Single-Domain Analytics Agents:** `115-snowflake-cortex-agents-core.md` Section 1.2
- **Multi-Domain Analytics Agents:** `115-snowflake-cortex-agents-core.md` Section 1.1
- **Hybrid Agents:** `115-snowflake-cortex-agents-core.md` Section 1.4
- **Planning Instructions:** `115-snowflake-cortex-agents-core.md` Section 4
...
- `106-snowflake-semantic-views-core` - For creating semantic views used by Cortex Analyst
- `115-snowflake-cortex-agents-core` - When using Cortex Analyst as an agent tool
- `106b-snowflake-semantic-views-querying` - For understanding semantic view query patterns

**Sometimes Related** (load if specific scenario):
- `106a-snowflake-semantic-views-advanced` - When troubleshooting semantic view validation issues
- `111-snowflake-observability-core` - When monitoring Cortex Analyst query performance
- `114-snowflake-cortex-aisql` - For SQL-based AI function usage (different approach than Analyst)

**Complementary** (different aspects of same domain):
- `107-snowflake-security-governance` - For masking policies and row access on semantic views
- `105-snowflake-cost-governance` - For monitoring Cortex Analyst costs
