...
### Mandatory
- MUST validate with `make rules-validate` before committing any rule changes
...
- MUST run `make lint` and `make format` before submitting PR

### Forbidden
- Direct editing of deployed rule files outside rules/ directory
- Committing without validation (`make rules-validate`)
...
5. Validate with `make rules-validate` and `make lint`
...
- Tool availability checked in Makefile
- Conventional Commits format understood

**Success Criteria:**
- `make rules-validate` passes without critical errors
- `make lint` passes cleanly
- `make format` passes without changes
- CHANGELOG.md updated (if user-facing change)
- All commits follow Conventional Commits format
- Generated files included in PR (`make rules-validate` run)
...
- [ ] Validation passed (`make rules-validate`, `make lint`)
...
2. **Check Makefile for available commands:** `make help` — verify validation commands exist
...
make rules-validate  # Ensure compliance
...
- Run the failing tests locally to reproduce: `make test` or equivalent
...
```
**Expected:** `make rules-validate` fails with schema version error.

### Test 2: Missing Required Section
Remove the `## Contract` section from a rule file.
**Expected:** `make rules-validate` fails with missing section error.
