...
102b-snowflake-sql-procedures.md tier=High kw=stored-procedure create-procedure udf create-function stored-procedure create-procedure create-function dollar-quoting nested-quotes execute-as execute-immediate bind-variables owner caller restricted-caller sql-scripting procedure-body
...
104-snowflake-streams-tasks.md tier=High kw=stream task cdc scheduled-tasks pipeline-automation merge-patterns sql task-dag after-dependencies task-history create-stream create-task debug-stream task-troubleshooting stream-consumption task-execution-error stream-lag
...
107-snowflake-security-governance.md tier=High kw=rbac grant roles grants secure-views security-policies access-control data-security policy-troubleshooting grant-management data-metric-functions dmf least-privilege create-masking-policy tagging sql dynamic-grant identifier
...
810-cli-design-core.md tier=Medium kw=command-line command-line-interface clig clig-dev cli-design cli-ux flags exit-codes stdout stderr no_color isatty tty subcommands cli-help cli-config xdg dry-run machine-readable
