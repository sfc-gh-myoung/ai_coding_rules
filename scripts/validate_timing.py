...
class TimingValidator:
    ...
    def __init__(self, review_file: Path):
        """Initialize with path to a review markdown file."""
    ...
    def validate(self) -> dict:
        ...
        if total_seconds and dimensions:
            ...
            if checkpoints:
                available_window = checkpoints.get("review_complete", 0) - checkpoints.get(
                    "skill_loaded", 0
                )
    ...
    def _extract_total_duration(self) -> float | None:
                    ...
                    if len(parts) >= 3:
                        ...
                        if "s" in duration_str:
                            try:
                                ...
                                dimensions.append(
                                    {"dimension": dim_name, "duration": duration, "mode": mode}
                                )
                            except ValueError:
                                pass
    ...
    def _validate_round_durations(self, dimensions: list[dict]):
        ...
        if len(round_durations) >= 3:
            ...
            self.warnings.append(f"Round duration detected: {', '.join(round_durations)}")
...
def main():
    """Validate timing data in one or more review files."""
    if len(sys.argv) < 2:
    ...
    for result in results:
        ...
        symbol = {"PASSED": "✓", "WARNING": "⚠", "FAILED": "✗", "SKIPPED": "-"}.get(status, "?")
