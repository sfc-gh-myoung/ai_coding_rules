...
**Expected grep pattern:** `grep -iE "fastapi|htmx|sse|spcs" rules/RULES_INDEX_COMPACT.md`
...
**Notes:** All 4 keywords must appear in the grep OR pattern. HTMX/SSE/SPCS may not match any rules in RULES_INDEX_COMPACT.md — note "No rules found for [keyword]" for each unmatched term.
