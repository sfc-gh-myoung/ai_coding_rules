    ...
    *,
    ...
    options = CortexCodeAgentOptions(
        ...
        system_prompt=(
            ...
            "`grep -iE 'KW1|KW2|KW3' rules/RULES_INDEX.md` to discover "
            "matching domain and activity rules.\n"
            "  4. Read every matched rule. For each rule's `Depends:` field, "
            "load `required:` deps and consider `optional:` deps based on "
            "context.\n"
            "  5. Use the `Read`, `Glob`, `Grep`, and `Bash` tools - do not "
            "answer from memory.\n\n"
            ...
            "Do NOT emit `**Bootstrap:**` headers or `Task Switch:` lines — "
            "those belong to a different (retired) contract.\n\n"
            ...
            "its own line and stop. Do NOT attempt to answer the user's "
            "underlying request, do NOT invoke domain skills, do NOT call "
            "ask_user_question, and do NOT call SQL or any other tool after "
            "`**Rules Loaded**` is written. This is a rule-discovery probe, "
            "not a task-completion run."
        ),
