**Description:** Advanced patterns for Snowflake Semantic Views including anti-patterns, validation rules, quality checks, and compliance requirements.
**Type:** Agent Requested
**AppliesTo:** `**/*.sql`, `**/*.yaml`, `**/*.yml`
**AutoAttach:** false
**Keywords:** Semantic views, validation, anti-patterns, quality checks, compliance, best practices, common mistakes, validation rules, semantic model quality, semantic view pitfalls, debug semantic view, fix semantic view errors, validation failures, relationship errors, mapping errors, quality assurance, semantic view testing, validation patterns
**TokenBudget:** ~5000
**ContextTier:** High
**Version:** 3.0
**LastUpdated:** 2025-11-20
**Depends:** 100-snowflake-core, 106-snowflake-semantic-views-core
...
## Rule Type and Scope
- **Type:** Agent Requested
- **Scope:** Anti-patterns, validation rules, quality checks, compliance

## Contract
- **Inputs/Prereqs:** Understanding of semantic view basics (from 106-core), semantic view created
- **Allowed Tools:** Validation queries, compliance checks, quality tools
- **Forbidden Tools:** None specific
- **Required Steps:** 1) Review anti-patterns 2) Apply validation rules 3) Run quality checks 4) Verify compliance
- **Output Format:** Validation queries, compliance checklists, quality reports
- **Validation Steps:** No anti-patterns detected; validation passes; compliant with requirements
...
**Pitfall 1: Reversed Mapping Syntax** [WARNING]
- **Trigger words**: "physical_column AS logical_name", backwards mapping
- **Why critical**: Syntax errors prevent semantic view creation
- **Correct approach**: Always use `logical_name AS physical_column` format
- **Detection**: Review DIMENSIONS/MEASURES for mapping direction

**Pitfall 2: Using CAST/DATE_TRUNC in DIMENSIONS** [WARNING]
- **Trigger words**: "CAST(", "DATE_TRUNC(", function calls in DIMENSIONS block
- **Why critical**: DIMENSIONS require simple column references only - functions cause errors
- **Correct approach**: Use simple columns in DIMENSIONS; complex expressions go in MEASURES
- **Detection**: Search DIMENSIONS block for parentheses (indicating function calls)

**Pitfall 3: Circular Relationships in TABLES Block** [WARNING]
- **Trigger words**: "bidirectional joins", "A joins B joins A"
- **Why critical**: Circular references cause semantic view creation failures
- **Correct approach**: Design hierarchical table relationships (parent-child, not circular)
- **Detection**: Map table relationships; verify no cycles exist

**Pitfall 4: Missing Equals Sign in COMMENT Syntax** [WARNING]
- **Trigger words**: "COMMENT 'text'", comment without equals
- **Why critical**: Syntax error - Snowflake SQL requires `COMMENT = 'text'` format
- **Correct approach**: Always use `COMMENT = 'text'` with equals sign
- **Detection**: Search for "COMMENT" without following "=" character
...
- **Cortex Analyst**: `117-snowflake-cortex-analyst.md` - Natural language query patterns
