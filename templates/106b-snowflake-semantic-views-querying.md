**Description:** Query patterns and testing strategies for Snowflake Native Semantic Views using SEMANTIC_VIEW() function.
**Type:** Agent Requested
**AppliesTo:** `**/*.sql`
**AutoAttach:** false
**Keywords:** SEMANTIC_VIEW query, DIMENSIONS, METRICS, FACTS, WHERE clause, window functions, dimension compatibility, testing, validation, TPC-DS, performance optimization, aliases, granularity, query semantic view, semantic view results, query patterns, result processing, SEMANTIC_VIEW function, query errors, dimension filters
**TokenBudget:** ~5750
**ContextTier:** High
**Version:** 1.0
**LastUpdated:** 2025-11-15
**Depends:** 106-snowflake-semantic-views
...
- **Type:** Agent Requested
- **Scope:** Querying semantic views after creation, testing patterns, validation strategies
...
**Pitfall 1: Combining FACTS and METRICS in Same Query** [WARNING]
- **Trigger words**: "FACTS and METRICS together", both clauses in SELECT
- **Why critical**: Mutually exclusive - query will fail with error
- **Correct approach**: Use either FACTS or METRICS, never both in same query
- **Detection**: Check query for presence of both "FACTS(" and "METRICS("

**Pitfall 2: Missing DIMENSIONS, METRICS, or FACTS Clause** [WARNING]
- **Trigger words**: "empty SELECT", no clauses specified
- **Why critical**: At least ONE clause required - query fails without it
- **Correct approach**: Always specify at least one of DIMENSIONS, METRICS, or FACTS
- **Detection**: Check query has at least one of these clauses

**Pitfall 3: Filtering on Non-Returned Columns in WHERE** [WARNING]
- **Trigger words**: "WHERE column not in SELECT", filtering unavailable dimensions
- **Why critical**: Can only filter on columns returned in DIMENSIONS/FACTS/METRICS
- **Correct approach**: Include dimension in SELECT before filtering on it in WHERE
- **Detection**: Compare WHERE clause columns to DIMENSIONS/FACTS/METRICS returned

**Pitfall 4: Dimension Granularity Mismatch with Metrics** [WARNING]
- **Trigger words**: "aggregation mismatch", incompatible dimensions with metrics
- **Why critical**: Metrics defined at specific granularity - incompatible dimensions cause errors
- **Correct approach**: Check metric definition granularity; use compatible dimensions only
- **Detection**: Review metric PARTITION BY/ORDER BY requirements vs selected dimensions
