**Description:** Troubleshooting deployment issues for Snowflake applications (notebooks, Streamlit, UDFs) with diagnostic commands and anti-patterns
**Type:** Agent Requested
**AppliesTo:** `Taskfile.yml`, `task/**/*.yml`, `sql/operations/**/*.sql`, Snowflake application debugging
**AutoAttach:** false
...
**Depends:** 100-snowflake-core, 109-snowflake-notebooks, 101-snowflake-streamlit-core, 109b-snowflake-app-deployment-core
...
- **Type:** Agent Requested
- **Scope:** Debugging and troubleshooting deployment issues for notebooks, Streamlit apps, UDFs, and stored procedures in Snowflake

## Quick Start TL;DR (Essential Troubleshooting Patterns)
...
**FORBIDDEN:**
- **Forbidden Actions:** 
  - Making assumptions about stage paths without verification
  - Speculating on root causes without diagnostic commands
  - Suggesting manual UI fixes without automation
  - Recommending workarounds that hide underlying issues
...
- **Validation Steps:** 
  - **Stage Verification:** `LIST @stage` shows uncompressed files (.py not .py.gz)
  - **Path Alignment:** DESCRIBE output ROOT_LOCATION matches LIST output paths
  - **Object Exists:** SHOW command returns deployed application
  - **Functional Test:** Application loads in Snowsight without errors
  - **Content Fresh:** Application displays updated code after deploy
