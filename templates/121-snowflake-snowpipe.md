**Description:** Comprehensive best practices for Snowflake Snowpipe and Snowpipe Streaming for continuous, near-real-time data ingestion including auto-ingest, REST API, and streaming architectures.
...
- **Allowed Tools:** Snowflake DDL for pipes; cloud storage event configuration (SNS, Azure Event Grid, GCS Pub/Sub); Snowpipe REST API; Snowpipe Streaming SDK (Java, Python, .NET); monitoring queries
- **Forbidden Tools:** Using Snowpipe for bulk historical loads; mixing bulk COPY and Snowpipe on same files; undersized files (<1MB); Snowpipe without proper file sizing
- **Required Steps:**
...
- **Output Format:** Complete pipe DDL with comments; cloud event configuration; Snowpipe Streaming code examples; monitoring queries; cost tracking patterns
- **Validation Steps:** Pipe created successfully; files loading automatically (auto-ingest) or via API; load history accessible; latency within requirements; error notifications working; cost tracking functional
