...
def _rule(filename: str, tier: str = "High", budget: int = 1000) -> RuleEntry:
    return RuleEntry(
        filename=filename,
        path=Path(filename),
        context_tier=tier,
...
class TestBuildManifest:
    def test_schema_version(self):
        ...
        assert manifest["schema_version"] == "rule-loader-manifest/v2"
    ...
    def test_dependency_only_rules_not_capped(self):
        ...
        non_dep = [r for r in manifest["load_sequence"] if not r.get("is_dependency_only")]
...
class TestManifestDict:
    ...
    def test_dependency_only_flag_emitted(self):
        ...
        dep_entry = next(e for e in manifest["load_sequence"] if e["filename"] == "000.md")
